...
> Note: this `closed_loop` path is the legacy/additive websocket compatibility path. For the current local Unity setup, external users should treat the ROS2-first path as the primary starting point. Older experimental closed-loop agent bridge prototypes have been moved to `archived/closed_loop-agent/`.
